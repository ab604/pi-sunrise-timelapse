#!/usr/bin/env python3
"""
Sunset Timelapse Script for Southampton, UK
...
CONFIG = {
    ...
    "capture": {
        ...
        "height": 800,
        "ev": 0.5,
        "start_before_sunset_minutes": 15,  # Start 15 min before sunset
    },
    "video": {
    ...
    "paths": {
        "base_dir": f'{os.path.expanduser("~")}/sunset_timelapse',
        "video_dir": f'{os.path.expanduser("~")}/sunset_timelapse/videos',
        "raw_dir": f'{os.path.expanduser("~")}/sunset_timelapse/raw_videos',
        "log_dir": f'{os.path.expanduser("~")}/sunset_timelapse/logs',
    },
...
class sunsetTimelapse:
    ...
    def setup_logging(self):
        ...
        log_file = log_dir / f"sunset_{today}.log"
    ...
    def setup_location(self):
        """Setup location for Sunset calculations"""
        loc = CONFIG['location']
    ...
    def get_sunset_time(self, date=None):
        """Get sunset time for given date (today if None)"""
        if date is None:
        ...
        try:
            if ASTRAL_VERSION == "new":
                ...
                sunset_local = sunset_utc.astimezone(local_tz).replace(tzinfo=None)
            else:
                # Older astral API should handle this automatically
                sunset_utc = self.location.sunset(date)
                import zoneinfo
                local_tz = zoneinfo.ZoneInfo('Europe/London')
                sunset_local = sunset_utc.astimezone(local_tz).replace(tzinfo=None)

            self.logger.info(f"sunset time for {date}: {sunset_local.strftime('%H:%M:%S')}")
            return sunset_local

        except Exception as e:
            self.logger.error(f"Error calculating sunset time: {e}")
            # Fallback: assume 16:00
            fallback_time = datetime.datetime.combine(date, datetime.time(16, 0))
            self.logger.warning(f"Using fallback sunset time: {fallback_time}")
    ...
    def capture_sunset_video(self):
        """Capture 75-minute sunset as continuous video"""
        sunset_time = self.get_sunset_time()
        capture_config = CONFIG['capture']

        # Calculate start time (45 minutes before sunset)
        start_offset = datetime.timedelta(minutes=capture_config['start_before_sunset_minutes'])
        start_time = sunset_time - start_offset
        end_time = start_time + datetime.timedelta(minutes=capture_config['duration_minutes'])

        # Create raw video file for today
        today = datetime.date.today().strftime('%Y-%m-%d')
        raw_dir = Path(CONFIG['paths']['raw_dir'])
        raw_video_path = raw_dir / f"sunset_raw_{today}.h264"

        self.logger.info(f"=== sunset Video Capture ===")
        self.logger.info(f"Date: {today}")
        self.logger.info(f"sunset time: {sunset_time.strftime('%H:%M:%S')}")
    ...
    def create_timelapse_from_video(self, raw_video_path):
        ...
        final_video_path = video_dir / f"sunset_{today}.mp4"
    ...
    def generate_ai_description(self, image_path):
        ...
        if not groq_key or groq_key == 'api_key':
            ...
            return "Sunset in Southampton. Again."
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Describe this image of sunset in Southampton in less than 250 characters.",
                            },
                            {
                                "type": "image_url",
                                "image_url": {
    ...
    def post_to_bluesky(self, video_path, description, sunset_time):
        ...
        try:
            ...
            post_result = client.create_post_with_video(
                ...
                alt_text="Southampton sunset timelapse"
            )
    ...
    def cleanup_old_files(self):
        ...
        raw_dir = Path(CONFIG['paths']['raw_dir'])
        removed_raw = 0
        for item in raw_dir.glob("sunset_raw_*.h264"):
            try:
                date_str = item.stem.replace('sunset_raw_', '')
...
def main():
    ...
    print("Southampton sunset Timelapse (Video Method)")
    print("=============================================================")
    print("✅ All bugs fixed: testing code removed, timeouts added, exception handling fixed")

    timelapse = sunsetTimelapse()

    try:
        # Show today's sunset info
        sunset_time = timelapse.get_sunset_time()
        start_time = sunset_time - datetime.timedelta(minutes=CONFIG['capture']['start_before_sunset_minutes'])

        print(f"Today's sunset: {sunset_time.strftime('%H:%M:%S')}")
        ...
        # Capture sunset as video
        raw_video_path = timelapse.capture_sunset_video()
        ...
        if final_video_path and final_video_path.exists():
            ...
            posted = timelapse.post_to_bluesky(final_video_path, description,sunset_time)
            ...
            timelapse.logger.info("sunset timelapse process completed successfully!")
